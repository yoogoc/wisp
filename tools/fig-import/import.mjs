...
const checkout = resolve(process.argv[2] ?? "./src/..");
const source = join(checkout, "src");
const out = resolve(process.argv[3] ?? new URL("./out/", import.meta.url).pathname);
...
const report = {
  source: `withfig/autocomplete ${manifest.version}`,
