...
impl Native {
    pub fn suggest(self, cwd: &Path) -> Vec<Suggestion> {
        match self {
...
fn history_in(path: &Path) -> Vec<Suggestion> {
...
mod tests {
    ...
    fn history_is_most_recent_first_without_zsh_metadata() {
        ...
        assert_eq!(names(&history_in(&path)), ["git status", "cargo test"]);
    }
}
